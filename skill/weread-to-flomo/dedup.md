...
### 4.4 批量 probe 的索引延迟（重要）

**实证**：`memo_search` 的**标签过滤路径**对刚写入的 memo 存在显著索引延迟。具体表现：

| Probe 形态 | 对刚写入 memo 是否立即可见 |
|------------|---------------------------|
| `keywords: "wr:hl:<完整 ID>"`（单条精确） | ✓ 立即（秒级） |
| `tag: "微信读书/{书名}"` + `keywords: "wr:hl"`（批量） | ✗ 至少几十分钟内不可见 |
| `keywords: "<书名>"`（全文检索） | ✗ 同样不可见 |

直接后果：

- 用户在**同一会话或短时间内**对同一本书连续跑两次导出 → 第二次的 §4 批量 probe 看不到第一次刚写的 memo → **重复写入**
- 跨会话的导出受影响小（间隔通常足够长）；但「一本书没导完中途中断 → 立即重启」的场景一定踩雷

**规避策略（必做）**：

批量 probe 拿到 `existing` 集合后，**在 agent 内存里再叠加一层 in-session 已写集合**：

```js
// 启动时初始化
const existing = new Set();   // 来自 memo_search 批量 probe（可能滞后）
const inSession = new Set();  // 本次会话写过的 (类型, ID) 二元组

// 每条待写入条目
const key = `${type}:${id}`;     // 如 "hl:41598972_3_691-761"
if (existing.has(key) || inSession.has(key)) continue;  // 跳过

await memo_create({ content });
inSession.add(key);              // 立刻加入内存集合
```

这层内存集合在写入后**立即**生效，不依赖 search 索引。

**额外保险（高代价、可选）**：对每条**待写入**条目，额外做一次 §3 单条精确 probe（`keywords: "wr:hl:<完整 ID>"`，索引延迟很小）。在「中断后立即重跑」场景下值得；常规跑增加 N 次 MCP 调用，按需开关。
...
| 刚写入的 memo 在批量 probe 里不可见 | 见 §4.4「批量 probe 的索引延迟」。**单条 probe 立即可见**，批量（标签 / 全文）至少几十分钟可能滞后；防御措施是 in-session 内存集合叠加 |
...
- [ ] **维护了 in-session 内存集合**（§4.4），在批量 probe 之上叠加防止刚写入条目被批量 probe 漏掉造成重复写？
